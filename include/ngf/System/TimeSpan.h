...
private:
  ...
  static const int64_t MaxSeconds =
      std::numeric_limits<int64_t>::max() / TicksPerSecond;
  static const int64_t MinSeconds =
      std::numeric_limits<int64_t>::min() / TicksPerSecond;

  static const int64_t MaxMilliSeconds =
      std::numeric_limits<int64_t>::max() / TicksPerMillisecond;
  static const int64_t MinMilliSeconds =
      std::numeric_limits<int64_t>::min() / TicksPerMillisecond;
...
public:
  ...
  explicit TimeSpan(int64_t ticks) noexcept: m_ticks(ticks) {
  }
  ...
  [[nodiscard]] int64_t getTicks() const {
    return m_ticks;
  }
...
private:
  static int64_t toTicks(int hour, int minute, int second) {
...
private:
  static TimeSpan interval(float value, int scale) {
    ...
    if ((millis > std::numeric_limits<int64_t>::max() / TicksPerMillisecond) ||
        (millis < std::numeric_limits<int64_t>::min() / TicksPerMillisecond))
      throw std::overflow_error("Specifed time is tool int64_t");
...
private:
  int64_t m_ticks{0}; ///< The number of ticks that represent the value of the current TimeSpan.
};
